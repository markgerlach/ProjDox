...
{
	[Serializable]
    public class mgModel_BaseObject : IDisposable, 
		IClassGenCloneable, IClassGenPropertyChanged, INotifyPropertyChanged
    {
		...
		public bool IsValid
		{
			get 
			{ 
				if (this._disposed) { throw new ObjectDisposedException("mgModel_BaseObject", _objectDisposedMessage); }
        ...
        public RecordStatus RecordStatus
        {
			...
			{ 
				...
				if (value != RecordStatus.Current)
				{
					...
					if (!RecordStatusParentHolders.TryGetValue(this.GetType(), out props))
					{
						...
						foreach (PropertyInfo prop in this.GetType().GetProperties())
						{
							if (prop.Name.EndsWith("Item") &&
								prop.PropertyType.FullName.Contains("_BaseWinProject.") &&
					...
					{
						if ((mgModel_BaseObject)prop.GetValue(this, null) != null)
						{
							switch (((mgModel_BaseObject)prop.GetValue(this, null)).RecordStatus)
							{
								case RecordStatus.Current:
									((mgModel_BaseObject)prop.GetValue(this, null)).RecordStatus = RecordStatus.Modified;
									break;
								default:
									((mgModel_BaseObject)prop.GetValue(this, null)).RecordStatus =
										((mgModel_BaseObject)prop.GetValue(this, null)).RecordStatus;
									break;
							}
						}
		...
		public void SetModifiedStatus(string changedPropName)
		{
			...
			{
				_changedProps.Add(changedPropName);
		...
		//{
		//    if (!mgModel_BaseObject.PropInfoArray.ContainsKey(typeName))
		//    {
		//        // Load them up
		//        Type t = System.Type.GetType("_BaseWinProject." + typeName);
		...
		//            }
		//        //}
		//        //((mgModel_BaseObject)o).Dispose();		// Dispose of the object
		//        mgModel_BaseObject.PropInfoArray.Add(typeName, newProps);		// Add the new collection
		//    }
		//}
			...
			bool deleteTopLevelRecord)
		{
			...
			try
			{
				...
				if ((recGUID is Int16 ||
					recGUID is Int32 ||
					recGUID is Int64) &&
					int.TryParse((string)recGUID, out val))
				{
...
#region ClassGenBindingList 
	...
	public interface IClassGenBindingList<T, K> where T : mgModel_BaseObject
	{
		// Methods
		...
		where T : mgModel_BaseObject
	{
		...
		protected bool _isLoaded = false;		// For use with parent and child collections
		protected DateTime? _dateTimeObjectPopulated = null;		// The Date/Time the object was populated
		protected RefreshFrequency _refreshRate = RefreshFrequency.ByDeveloperOnly;			// Set the refresh rate
		protected ClassGenRules _rules = null;
		[NonSerialized, System.Xml.Serialization.XmlIgnore]
		protected SqlConnection _asyncConnection = null;		// A connection that can be used for updates
		...
		public new void Insert(int index, T item)
		{
			...
			{
				// Log the error
				throw new _BaseWinProjectException("The key: " + ((K)item.GetType().GetProperty(KeyFieldName).GetValue(item, null)).ToString() +
		...
		public bool IsCurrent
		{
			get 
			{
				if (!(typeof(mgModel_BaseObject).IsAssignableFrom(typeof(T))))
				{
					return true;
				}
				foreach (T item in this)
				{
					if ((item as mgModel_BaseObject).RecordStatus != RecordStatus.Current)
					{
						return false;
					}
				}
		...
		public List<string> PropertyValueExistsMoreThanOnceInCollection(string propertyName, bool sortDescending)
		{
			...
			foreach (T item in this)
			{
				if ((item as mgModel_BaseObject).RecordStatus != RecordStatus.Deleted)
				{
